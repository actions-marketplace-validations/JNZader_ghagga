...
describe('ghagga hooks install', () => {
  it('installs both hooks by default', async () => {
    ...
    expect(mockInstallHook).toHaveBeenCalledWith(
      '/repo/.git/hooks', 'pre-commit', 'pre-commit-content', false,
    );
    expect(mockInstallHook).toHaveBeenCalledWith(
      '/repo/.git/hooks', 'commit-msg', 'commit-msg-content', false,
    );
  ...
  it('passes force flag when --force is used', async () => {
    ...
    expect(mockInstallHook).toHaveBeenCalledWith(
      '/repo/.git/hooks', 'pre-commit', 'pre-commit-content', true,
    );
  });
  ...
  it('shows error message when install fails', async () => {
    ...
    expect(tui.log.success).toHaveBeenCalledWith(
      expect.stringContaining('Installed commit-msg'),
    );
  });
  ...
  it('shows "no hooks installed" warning when all installs fail', async () => {
    ...
    expect(tui.log.warn).toHaveBeenCalledWith(
      expect.stringContaining('No hooks were installed'),
    );
  });
  ...
  it('shows install count summary on success', async () => {
    ...
    expect(tui.log.info).toHaveBeenCalledWith(
      expect.stringContaining('Installed 2 hook(s)'),
    );
  });
});
