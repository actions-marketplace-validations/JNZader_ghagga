...
export function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-surface-bg">
      {/* Sidebar */}
      <aside className="flex w-64 flex-col border-r border-surface-border bg-surface-card">
        {/* Logo */}
        <div className="flex h-16 items-center gap-3 border-b border-surface-border px-6">
          <span className="text-2xl" role="img" aria-label="GHAGGA bird">
            🐦
          </span>
          <div>
            <h1 className="text-lg font-bold text-text-primary">GHAGGA</h1>
            <p className="text-xs text-text-secondary">AI Code Review</p>
          </div>
        </div>
        ...
        {user && (
          <div className="border-t border-surface-border p-4">
            <div className="flex items-center gap-3">
              <img src={user.avatarUrl} alt={user.githubLogin} className="h-8 w-8 rounded-full" />
