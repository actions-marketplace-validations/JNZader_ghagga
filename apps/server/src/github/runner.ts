 ...
 *
 * Manages the per-user/org public runner repo that executes static analysis
 * tools (Semgrep, Trivy, PMD/CPD) via GitHub Actions. This offloads the
 * memory-heavy tools from the Render server (512MB) to Actions runners (7GB).
 *
 * Public API:
...
export async function ensureRunnerRepo(
  ...
  if (!exists) {
    logger.info({ owner }, 'Creating runner repo');
    const isOrg = await isOrganization(owner, token);
    await createRepo(owner, token, isOrg);
  ...
  if (!integrity.valid) {
    logger.warn({ owner }, 'Workflow tampered — re-committing canonical version');
...
> {
  ...
  if (!exists) {
    try {
      ...
      logger.error({ owner, error: msg }, 'Failed to create runner repo');
    ...
    const integrity = await verifyWorkflowIntegrity(owner, token);
    if (!integrity.valid) {
      logger.warn({ owner }, 'Workflow tampered — re-committing before dispatch');
      try {
        await commitWorkflowFile(owner, token, integrity.existingSha);
        // Re-verify after commit
        const recheck = await verifyWorkflowIntegrity(owner, token);
        if (!recheck.valid) {
  ...
  // Set installation token as repo secret
  try {
    await setRepoSecret(owner, 'GHAGGA_TOKEN', token, token);
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    logger.error({ owner, error: msg }, 'Failed to set GHAGGA_TOKEN secret');
  ...
  const dispatchRes = await fetch(
    `${GITHUB_API}/repos/${owner}/${RUNNER_REPO_NAME}/dispatches`,
    {
      method: 'POST',
  ...
  logger.info({ owner, callbackId, prNumber: context.prNumber }, 'Analysis dispatched');
...
export async function deleteRunnerRepo(
  ...
  try {
    const token = await getInstallationToken(installationId, appId, privateKey);
    const res = await fetch(`${GITHUB_API}/repos/${owner}/${RUNNER_REPO_NAME}`, {
      method: 'DELETE',
      headers: { ...API_HEADERS, Authorization: `Bearer ${token}` },
    });
    if (res.ok || res.status === 404) {
      logger.info({ owner }, 'Runner repo deleted');
    } else {
      logger.warn({ owner, status: res.status }, 'Failed to delete runner repo');
    }
  } catch (err) {
    logger.warn({ owner, error: err instanceof Error ? err.message : String(err) }, 'Runner repo deletion error');
  }
}
...
async function isOrganization(owner: string, token: string): Promise<boolean> {
...
async function createRepo(owner: string, token: string, isOrg: boolean): Promise<void> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      description: 'GHAGGA AI Code Review — static analysis runner (auto-managed, do not modify)',
      private: false,
      auto_init: true,
      has_issues: false,
      has_projects: false,
...
async function commitWorkflowFile(
  ...
  const res = await fetch(
    `${GITHUB_API}/repos/${owner}/${RUNNER_REPO_NAME}/contents/${WORKFLOW_PATH}`,
    {
      method: 'PUT',
      headers: { ...API_HEADERS, Authorization: `Bearer ${token}` },
...
async function setLogRetention(owner: string, token: string): Promise<void> {
  ...
  try {
    const retentionRes = await fetch(
      `${GITHUB_API}/repos/${owner}/${RUNNER_REPO_NAME}/actions/permissions/artifact-and-log-retention`,
      {
        method: 'PUT',
        headers: { ...API_HEADERS, Authorization: `Bearer ${token}` },
  ...
  try {
    const permRes = await fetch(
      `${GITHUB_API}/repos/${owner}/${RUNNER_REPO_NAME}/actions/permissions`,
      {
        method: 'PUT',
        headers: { ...API_HEADERS, Authorization: `Bearer ${token}` },
        body: JSON.stringify({
